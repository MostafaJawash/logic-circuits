...
const STORAGE_KEY = "chapter-4-quiz-result";
...
/** يخلط ترتيب الأسئلة وترتيب خياراتها في كل محاولة */
function prepare(): PreparedQuestion[] {
  return shuffle(sourceQuestions).map((q) => {
...
export function Quiz() {
  ...
  if (finished) {
    return <Results quizItems={quizItems} answers={answers} seconds={seconds} onRestart={reset} />;
  }
    ...
    <div>
      ...
      {lastResult && (
        <motion.div
          ...
          className="mb-5 flex items-center justify-center gap-2 rounded-xl border border-border-soft/70 bg-card/40 px-4 py-2.5 text-sm text-muted"
        >
          <History className="h-4 w-4 text-accent" />
          آخر محاولة: <span className="font-bold text-accent">{lastResult.percentage}%</span> (
                ...
                >
                  <span
                    className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-sm font-bold transition-colors ${
                      active ? "bg-accent text-background" : "bg-secondary/70 text-muted"
                    }`}
                  >
                    {["أ", "ب", "ج", "د"][i]}
                  </span>
...
function Results({
  ...
  const stats = useMemo(() => {
    ...
    return { correct, wrong, skipped, total, percentage, weakSections, strongSections };
  }, [quizItems, answers]);
  ...
  const motivation =
    percentage >= 90
      ? {
          text: "ممتاز 🔥",
          note: "أداء رائع، إتقان شبه كامل للفصل.",
          icon: Flame,
      ...
      : percentage >= 75
        ? {
            text: "جيد جداً 👏",
            note: "مستوى قوي، راجع بعض التفاصيل لتصل للكمال.",
            icon: ThumbsUp,
            tone: "from-sky-500/20 to-blue-500/10 border-sky-500/30 text-sky-300",
          }
        : percentage >= 50
          ? {
              text: "استمر بالتدريب 💪",
              note: "أساس جيد، راجع الفقرات الضعيفة وأعد المحاولة.",
              icon: Dumbbell,
              tone: "from-amber-500/20 to-orange-500/10 border-amber-500/30 text-amber-300",
            }
          : {
              text: "راجع بعض الفقرات وستتقنها.",
              note: "لا بأس، ابدأ بمراجعة الفقرات المقترحة ثم أعد الاختبار.",
              icon: BookOpen,
              tone: "from-rose-500/20 to-pink-500/10 border-rose-500/30 text-rose-300",
            };

  const understanding = percentage >= 85 ? "عالٍ" : percentage >= 60 ? "متوسط" : "يحتاج تحسين";
  ...
  return (
    ...
    >
      ...
      <motion.div variants={fadeUp} className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <AnalyticCard
          icon={Target}
          title="⚠️ الفقرات التي تحتاج مراجعة"
          ...
          {weakSections.length > 0 ? (
            <ul className="space-y-1.5">
              {weakSections.map((s) => (
                <li
                  key={s.section}
                  className="flex items-center justify-between gap-2 text-amber-200"
                >
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <MeterCard icon={BrainCircuit} title="🧠 مستوى الفهم" value={understanding} percentage={percentage} />
        <MeterCard icon={TrendingUp} title="📈 مستوى الاستيعاب" value={understanding} percentage={percentage} />
      </div>
      ...
      <div className="mt-6 flex flex-wrap justify-center gap-3">
        ...
        <button
          onClick={onRestart}
          className="inline-flex items-center gap-2 rounded-xl border border-accent/40 bg-accent/10 px-6 py-3 text-sm font-bold text-accent transition-all hover:bg-accent/20"
        >
